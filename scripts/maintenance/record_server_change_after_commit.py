...
LOG = REPO / 'docs/overview/\u670d\u52a1\u5668\u53d8\u66f4\u65e5\u5fd7.md'
...
def build_entry(meta: dict[str, str], key_files: list[str]) -> str:
    ...
    files_text = '\u3001'.join(f'`{item}`' for item in key_files) if key_files else '\u89c1 git \u63d0\u4ea4'
    lines = [
        f'## {title}',
        '',
        f'- \u65f6\u95f4\uff1a{title}',
...
def main() -> int:
    ...
    log_file = 'docs/overview/\u670d\u52a1\u5668\u53d8\u66f4\u65e5\u5fd7.md'.encode('utf-8').decode('unicode_escape')
    if changed_files and set(changed_files).issubset({log_file}):
